...
const props = defineProps({
  isDialogOpen: Boolean, // This allows v-model to bind to isDialogOpen
});
const isDialogOpen = computed(() => { return props.isDialogOpen })

const { t } = useI18n({ useScope: 'global' })
const emit = defineEmits(['updateFilter', 'update:isDialogOpen']);

const datePicker = ref(null);

// Function to toggle dialog visibility
const toggleDialog = () => {
  emit('update:isDialogOpen', !props.isDialogOpen);
};
