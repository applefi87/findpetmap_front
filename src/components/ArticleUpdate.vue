<!-- 編輯文章的介面 -->
<template>
  <q-form id="articleUpdate" @submit.prevent="handleUpdateArticle" style="width:90vw">
    ...
    <!--  -->
    <q-btn type="submit" class="q-mt-md" :label="t('save')" :loading="isUpdating" color="primary" />
  </q-form>
</template>
...
import * as anv from 'an-validator';
...
const handleUpdateArticle = async () => {
  try {
    ...
    const res = await articleService.updateArticle(props.article._id, articleForm, images.value)
    if (res.success) {
